...
* `addConnection`/`deleteConnection`: functions that help create new or destroy existing connections; passed directly to InPorts.
...
* `addConnection`/`deleteConnection`: functions that help create new or destroy existing connections.
* `connectDropTarget`: a function generated by React-dnd that allows the developer to register a JSX object to be a drop target.
...
Connections represent links between nodes. They connect port to port, and for aesthetic reasons have a set of rules that are followed when being displayed. There may be some bugs here, since there are too many scenarios.

Connection objects are decoupled from the rest of the editor as much as possible: they only know where they start, where they end, and the height of the nodes that they are connecting. The height is used to calculate which rule to follow when displaying the connection.

Connections are used in the custom drag layer (in addition to in the editor) to display a preview of what a connection will look like as its being made.

##### Props

* `sX`/`sY`: integers that represent the pixel location of the starting port.
* `eX`/`eY`: integers that represent the pixel location of the ending port.
* `height`: height of the nodes that are being connected (currently they must be the same number).

#### EditorDragLayer

The EditorDragLayer allows for customizable drag previews when dragging either nodes or creating new connections. Currently, the drag layer only works on creating new connections: a phantom connection line will be made between the starting point and the cursor's current position, constantly updating as the cursor moves. There are plans to create a custom drag preview for nodes so that the node follows the cursor as it is dragged.

##### Props

* `item`: unused; represents the item being dragged.
* `itemType`: allows us to differentiate between which preview to display.
* `initialOffset`/`currentOffset`: used in tandem to calculate how far the object has been dragged.
* `initialPosition`: initial location of the cursor before dragging commenced.
* `isDragging`: boolean representing whether or not the drag layer is necessary.

### Constants

There are many string constants used throughout this project, and they are all conveniently stored in a single file.

* `ItemTypes`: used by React-dnd to differentiate between types of drag sources.
* `MenuTypes`: used by React-contextmenu to differentiate between types of right click menus.
* `NodeNames`: passed around in the backend to specify which node type is being referred to. They act as keys for the following constants.
* `ProperNames`: formatted, displayed strings
* `SourceFiles`: file path strings 
